  ...
  <section class="py-12 sm:py-16 lg:py-20">
    <div class="px-4 mx-auto sm:px-6 lg:px-8 max-w-7xl">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 sm:text-3xl">Top Athletes</h2>
        <p class="text-gray-500 font-normal text-base mt-2.5">Current top 5 - {{ today }}</p>
      </div>
      ...
      <div class="grid grid-cols-1 gap-5 mt-8 text-center sm:mt-12 sm:grid-cols-2 lg:grid-cols-5 sm:gap-6">
        <div v-for="(athlete, index) in topList" :key="index" class="relative transition-all duration-300 bg-gray-100 rounded-xl hover:shadow-xl hover:bg-white">
          <div class="px-4 py-5 sm:p-6">
            ...
            <p class="mt-3 font-medium text-gray-500">{{ athlete.height }} - {{ athlete.weight }}</p>
            <p class="mt-2 mb-5 font-medium text-gray-500">{{ athlete.gender }} - {{ athlete.age }} Years</p>
            <p v-for="(score, indexS) in athlete.scores" :key="indexS" class="mt-2 mb-5 font-medium text-gray-500">
              <bold class="font-bold text-gray-900"> 22.{{ score.ordinal }} </bold> - {{ score.scoreDisplay }} - Nº{{ score.rank }}
            </p>
      ...
      <div class="mt-12 text-center">
        <a
          href="https://games.crossfit.com/leaderboard/open/2022?view=0&division=1&region=0&scaled=0&sort=0"
          target="_blank"
          title="To official website"
          class="inline-flex items-center p-1 -m-1 text-xs font-bold tracking-wide text-gray-400 uppercase transition-all duration-200 rounded hover:text-gray-900 focus:ring-2 focus:text-gray-900 focus:ring-gray-900 focus:ring-offset-2 focus:outline-none"
          role="button"
        >
          Browse All
